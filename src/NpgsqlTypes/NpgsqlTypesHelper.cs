...
internal class NpgsqlTypesHelper
{
    ...
    public static Hashtable LoadTypesMapping(NpgsqlConnection conn)
    {
        ...
        lock(typeof(NpgsqlTypesHelper))
        {
            ...
            NpgsqlCommand command = new NpgsqlCommand("select oid, typname from pg_type where typname in ('bool', 'bytea', 'date', 'float4', 'float8', 'int2', 'int4', 'int8', 'numeric', 'text', 'time', 'timestamp', 'timestamptz');", conn);
